/** Proof in Motion — reusable contribution evidence instrument; retain the compact source-led visual calendar and expose only a restrained manual live-data refresh control. */
...
export function ContributionCalendar() {
  const { calendar, refresh, refreshState } = useContributionCalendar();
  ...
  const refreshLabel = refreshState === "refreshing" ? "REFRESHING" : "REFRESH LIVE DATA";
  const refreshStatus = refreshState === "ready"
    ? "LIVE DATA REFRESHED"
  ...
  return <section className="activity-calendar" aria-label={`${calendar.total} public GitHub contributions in the last year`}>
    ...
    <div className="activity-calendar__sources"><a href="https://github.com/TheCyperpunk" target="_blank" rel="noreferrer"><b>S//</b> @TheCyperpunk / public profile <ArrowUpRight size={13} /></a><span>{sourceLabel}</span><button className="activity-calendar__refresh" type="button" onClick={() => { void refresh(); }} disabled={refreshState === "refreshing"} aria-label="Refresh live GitHub contribution data"><RefreshCw size={12} className={refreshState === "refreshing" ? "activity-calendar__refresh-icon--spinning" : undefined} /><span>{refreshLabel}</span></button>{refreshStatus ? <span className="activity-calendar__refresh-status" aria-live="polite">{refreshStatus}</span> : null}<span>LIVE EVENT LEDGER BELOW</span></div>
  </section>;
}
