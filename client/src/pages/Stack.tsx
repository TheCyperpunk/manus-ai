...
import { organizationRepositoryAudit } from "@/lib/organization-stack-repo-audit";
...
// Proof in Motion — hide only the user-identified entries that do not communicate a substantive project stack; retain all other audited personal and organization-owned repository evidence.
...
const personalProjectStackLedger = stackRepositoryAudit.repositories
...
const organizationProjectStackLedger = organizationRepositoryAudit.repositories.map((repository, index) => ({
...
const projectStackLedger = [...personalProjectStackLedger, ...organizationProjectStackLedger];
...
export default function Stack() {
  ...
  return <main className="atlas-main atlas-page">
    ...
    <section className="stack-ledger" aria-labelledby="stack-ledger-title">
      <header className="stack-ledger__head"><div><p className="atlas-kicker"><i />PROJECT STACK LEDGER</p><h2 id="stack-ledger-title">Stack use,<br /><em>by project.</em></h2></div><p>Each selected repository row carries public GitHub source and primary-language evidence; six personal project rows retain the deeper code, manifest, and README audit. Organization-owned rows are explicitly labeled with their organization, preserve fork status where applicable, and are included only after the same public-source review. Entries without a substantive project stack signal remain omitted.</p></header>
      ...
      <a className="stack-ledger__source" href="https://github.com/TheCyperpunk" target="_blank" rel="noreferrer"><Waypoints size={15} /><span>GITHUB SOURCE</span><b>{personalProjectStackLedger.length} selected personal repositories from {stackRepositoryAudit.eligibleRepositoryCount} original public repositories · {organizationProjectStackLedger.length} verified organization projects across {organizationRepositoryAudit.suppliedOrganizationCount} supplied organizations · 6 deep code/manifest/README audits</b><ArrowUpRight size={15} /></a>
    </section>
