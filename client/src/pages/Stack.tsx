...
export default function Stack() {
  ...
  return <main className="atlas-main atlas-page">
    ...
    <section className="stack-instrument"><div className="stack-instrument__frame"><span>S// LANGUAGE INSTRUMENT</span><i /><b>MEASURED FROM PUBLIC METADATA</b></div><div className="stack-instrument__meta"><Code2 size={22} /><p>PRIMARY LANGUAGE DISTRIBUTION</p><strong>{metrics.original.length || "…"}</strong><span>original, non-archived repositories with public metadata</span></div><div className="language-bars"><div className="language-bars__stamp"><span>API // REPOSITORY INVENTORY</span><b>LIVE SOURCE</b></div>{metrics.languages.map((entry, index) => <article key={entry.language}><div><span>{String(index + 1).padStart(2, "0")}</span><b>{entry.language}</b><em>{entry.repositories} repos</em></div><i><u style={{ width: `${(entry.repositories / maximum) * 100}%` }} /></i></article>)}{source.status === "loading" && <p className="atlas-empty">Reading repository languages…</p>}</div></section>
