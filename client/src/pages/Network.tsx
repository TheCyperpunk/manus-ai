...
export default function Network() {
  ...
  const organizationDataAvailable = source.status !== "loading";
  const hasNoPublicOrganizations = source.orgs.length === 0;
  return <main className="atlas-main atlas-page">
    ...
    <section className="network-dossier"><article className="network-primary"><div className="network-primary__top"><Building2 size={22} /><span>PUBLIC ORGANIZATIONS</span></div>{hasNoPublicOrganizations ? <><strong>0</strong><h2>No public memberships<br />returned by GitHub.</h2><p>{source.status === "ready" ? "The live profile API reports no public organization memberships." : "Verified snapshot captured 19 Aug 2026 reports no public organization memberships."} This result does not make a statement about private teams or other affiliations.</p></> : <div className="org-list">{source.orgs.map((org) => <a href={org.html_url} target="_blank" rel="noreferrer" key={org.login}><img src={org.avatar_url} alt="" /><span><b>{org.login}</b><small>{org.description || "Public GitHub organization"}</small></span><ArrowUpRight size={15} /></a>)}</div>}</article><article className="network-secondary"><div className="network-primary__top"><NetworkIcon size={22} /><span>CROSS-REPOSITORY ACTIVITY</span></div><h2>Public contribution<br />targets in recent events.</h2><p>Derived from returned public GitHub events where the repository owner is not @TheCyperpunk. These are activity targets, not asserted memberships.</p><div className="external-list">{metrics.external.slice(0, 5).map((item, index) => <a href={`https://github.com/${item.owner}`} target="_blank" rel="noreferrer" key={item.owner}><span>0{index + 1}</span><b>{item.owner}</b><small>{item.events} returned events</small><ArrowUpRight size={14} /></a>)}{organizationDataAvailable && metrics.external.length === 0 && <p>No external-owner events were returned in the current public feed.</p>}</div></article></section>
